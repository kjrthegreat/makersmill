...
export function ProfileForm({ vendor, hours: initialHours, userId }: Props) {
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    if (vendor) {
      ...
      const { data: newVendor, error: vErr } = await supabase
        .from('vendors')
        .insert({ ...payload, slug: `${slug}-${userId.slice(0, 6)}`, user_id: userId, status: 'pending' })
        .select()
        .single();
      ...
      if (newVendor) {
        for (const h of hours) {
          await supabase.from('vendor_hours').insert({
  ...
  return (
    <form className="apply-form dash-form" onSubmit={handleSave}>
      ...
      {saved && <div className="form-success-inline">Profile saved successfully.</div>}
      ...
      {/* ── CATEGORIES ── */}
      <div className="dash-form-section">
        <h2 className="dash-form-section-h">Categories <em style={{ color: 'var(--gray)', fontFamily: 'var(--stamp)', fontSize: 9, letterSpacing: '.1em', fontStyle: 'normal', textTransform: 'uppercase' }}>select all that apply</em></h2>
      ...
      <div className="dash-form-section">
        ...
        <div className="form-row">
          <label>
            Hero Headline <em style={{ color: 'var(--gray)', fontStyle: 'normal' }}>overrides name-split default</em>
            <input
              type="text"
      ...
      <div className="dash-form-section">
        ...
        <label>
          Meta Description <em style={{ color: 'var(--gray)', fontStyle: 'normal', fontSize: 11 }}>shown in Google search results (max 160 chars)</em>
          <textarea
            value={form.meta_description}
            onChange={e => setField('meta_description', e.target.value)}
            placeholder="Describe your shop for search engines — what you sell, your style, where to find you."
            rows={3}
            maxLength={160}
          />
        </label>
      ...
      {/* ── HOURS ── */}
      <div className="dash-form-section">
        <h2 className="dash-form-section-h">Hours <em style={{ color: 'var(--gray)', fontFamily: 'var(--stamp)', fontSize: 9, letterSpacing: '.1em', fontStyle: 'normal', textTransform: 'uppercase' }}>leave blank to inherit mill hours</em></h2>
